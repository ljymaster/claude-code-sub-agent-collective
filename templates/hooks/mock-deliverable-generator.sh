...
LOG_FILE="/tmp/mock-deliverable-generator-${PLATFORM}.log"
timestamp() { date '+%Y-%m-%d %H:%M:%S'; }
log() { echo "[$(timestamp)] [$PLATFORM] $1" >> "$LOG_FILE"; }

EVENT=${EVENT:-""}
SUBAGENT_NAME=${SUBAGENT_NAME:-""}
AGENT_OUTPUT=${AGENT_OUTPUT:-""}

log "Mock Deliverable Generator - Platform: $PLATFORM, Event: $EVENT, Agent: $SUBAGENT_NAME"
