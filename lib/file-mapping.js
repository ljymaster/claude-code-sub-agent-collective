...
class FileMapping {
  ...
  getFileMapping() {
    return [
      ...
      // Native Claude 4.5 features (v3.0)
      ...this.getMemoryMapping(),
      ...this.getOutputStyleMapping(),
  ...
  getCollectiveMapping() {
    const collectiveFiles = [
      ...
      // DECISION.md removed in v3.0 - native routing handles this
      {
        file: 'task-system.md',
        required: true,
        description: 'Memory-based task breakdown and execution system'
      },
      {
        file: 'agents.md',
        required: true,
      ...
      {
        file: 'van-output-template.md',
        required: true,
        description: '/van workflow output format template'
      },
      {
        file: 'smoke-tests/run-all.sh',
        required: true,
        executable: true,
        description: 'Smoke test runner script'
      },
      {
        file: 'smoke-tests/tdd-gate-task-aware.test.sh',
        required: true,
        executable: true,
        description: 'TDD-gate task-aware validation smoke test'
      },
      {
        file: 'smoke-tests/preflight-validation.test.sh',
        required: true,
        executable: true,
        description: 'Preflight user confirmation validation smoke test'
      },
      {
        file: 'smoke-tests/wbs-helpers.test.sh',
        required: true,
        executable: true,
        description: 'WBS helper functions validation smoke test'
      },
      {
        file: 'smoke-tests/van-instructions.test.sh',
        required: true,
        executable: true,
        description: 'Van command instructions validation smoke test'
      },
      {
        file: 'smoke-tests/subagent-stop-reliability.test.sh',
        required: true,
        executable: true,
        description: 'SubagentStop hook reliability validation smoke test'
      },
      {
        file: 'smoke-tests/tdd-gate-completion.test.sh',
        required: true,
        executable: true,
        description: 'TDD-gate completion logic validation smoke test'
      },
      {
        file: 'smoke-tests/task-id-extraction.test.sh',
        required: true,
        executable: true,
        description: 'Task ID extraction reliability validation smoke test'
      },
      {
        file: 'smoke-tests/hook-error-propagation.test.sh',
        required: true,
        executable: true,
        description: 'Hook error propagation validation smoke test'
      },
      {
        file: 'smoke-tests/hub-and-spoke-language.test.sh',
        required: true,
        executable: true,
        description: 'Hub-and-spoke architectural language validation smoke test'
      },
      {
        file: 'smoke-tests/validation-loop-fix.test.sh',
        required: true,
        executable: true,
        description: 'Validation loop agent extraction fix smoke test'
      },
      {
        file: 'smoke-tests/preflight-user-confirmation.test.sh',
        required: true,
        executable: true,
        description: 'Preflight user confirmation via transcript validation smoke test'
      }
    ];
  ...
  getMemoryMapping() {
    return [
      {
        source: path.join('.claude', 'memory.md'),
        target: path.join(this.paths.claude, 'memory.md'),
        type: 'memory',
        required: true,
        overwrite: true,
        description: 'Native memory system - project-level behavioral rules'
      },
      // Deterministic memory library scripts
      {
        source: path.join('.claude', 'memory', 'lib', 'memory.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'memory.sh'),
        type: 'memory',
        required: true,
        overwrite: true,
        description: 'Deterministic memory operations (write/read/update)'
      },
      {
        source: path.join('.claude', 'memory', 'lib', 'wbs-helpers.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'wbs-helpers.sh'),
        type: 'memory',
        required: false,
        overwrite: true,
        description: 'WBS helpers for roll-up and hierarchy operations'
      },
      // Logging system
      {
        source: path.join('.claude', 'memory', 'lib', 'logging.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'logging.sh'),
        type: 'memory',
        required: true,
        overwrite: true,
        description: 'Deterministic logging library for hooks and memory operations'
      },
      {
        source: path.join('.claude', 'memory', 'lib', 'log-analysis.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'log-analysis.sh'),
        type: 'memory',
        required: true,
        overwrite: true,
        description: 'Log analysis and query tools'
      },
      // Logging control scripts (deterministic toggle management)
      {
        source: path.join('.claude', 'memory', 'lib', 'logging-enable.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'logging-enable.sh'),
        type: 'memory',
        required: true,
        overwrite: true,
        description: 'Enable logging system (creates toggle file deterministically)'
      },
      {
        source: path.join('.claude', 'memory', 'lib', 'logging-disable.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'logging-disable.sh'),
        type: 'memory',
        required: true,
        overwrite: true,
        description: 'Disable logging system (removes toggle file)'
      },
      // Browser testing preflight (deterministic notification)
      {
        source: path.join('.claude', 'memory', 'lib', 'browser-testing-preflight.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'browser-testing-preflight.sh'),
        type: 'memory',
        required: true,
        overwrite: true,
        description: 'Browser testing preflight check (detects UI, shows notification)'
      },
      {
        source: path.join('.claude', 'memory', 'lib', 'logging-status.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'logging-status.sh'),
        type: 'memory',
        required: true,
        overwrite: true,
        description: 'Show logging status and recent activity'
      },
      // Preflight configuration script (v3.0 spec-kit pattern)
      {
        source: path.join('scripts', 'preflight.sh'),
        target: path.join(this.paths.claude, 'memory', 'lib', 'preflight.sh'),
        type: 'script',
        required: true,
        overwrite: true,
        executable: true,
        description: 'Preflight configuration script (logging, browser testing, PRD path)'
      }
    ];
  }

  getOutputStyleMapping() {
    const styles = [
      'tdd-mode.md',
      'collective-mode.md',
      'research-mode.md'
    ...
    return styles.map(style => ({
      source: path.join('.claude', 'output-styles', style),
      target: path.join(this.paths.claude, 'output-styles', style),
      type: 'output-style',
      required: true,
      overwrite: true,
      description: `Output style: ${style.replace('.md', '')}`
    }));
  }
  ...
  getAgentMapping() {
    const allAgents = [
      ...
      'chrome-devtools-testing-agent.md',
      'command-system-agent.md',
      'completion-gate.md',
      'component-implementation-agent.md',
      'deliverables-validation-agent.md',  // v3.0: Intelligent deliverables validation at feature completion
      'devops-agent.md',
      ...
      'research-agent.md',
      'routing-agent.md',
      'task-breakdown-agent.md',         // v3.0: Replaces task-generator-agent
      'task-checker.md',
      'task-executor.md',
  ...
  getHookMapping() {
    const hooks = [
      // v3.0: Simplified hooks using native Claude 4.5 features
      {
        file: 'tdd-gate.sh',
        required: true,
        description: 'TDD enforcement using native decision control'
      },
      {
        file: 'session-init.sh',
        required: true,
        description: 'One-time session initialization message'
      },
      {
      ...
      {
        file: 'preflight-confirmation.sh',
        required: true,
        description: 'Validates preflight user confirmation via transcript (prevents LLM fabrication)'
      },
      {
        file: 'subagent-validation.sh',
        required: false,
        description: 'SubagentStop validation: tests + deliverables + status roll-up'
      },
      {
        file: 'pre-agent-deploy.sh',
        required: false,
        description: 'PreToolUse validation: dependency checking and leaf-task enforcement'
      }
      // Removed in v3.0:
      // - directive-enforcer.sh (native memory system handles this)
      // - routing-executor.sh (native subagent routing)
      // - load-behavioral-system.sh (native memory auto-loads)
      // - test-driven-handoff.sh (native SubagentStop events)
    ];
  ...
  getCommandMapping() {
    const commands = [
      ...
      'van.md',             // v3.0: Simplified van command
      'van-logging.md',     // v3.0: Logging system control
      'autocompact.md',
      'continue-handoff.md',
      'mock.md',
      'reset-handoff.md',

      // Memory-based task inspection commands (v3.0)
      'tasks/show/show-task.md',           // /tasks show <task-id>
      'tasks/status/project-status.md',    // /tasks status
      'tasks/next/next-task.md',           // /tasks next
      'tasks/list/list-tasks.md'           // /tasks list [filter]
    ];
