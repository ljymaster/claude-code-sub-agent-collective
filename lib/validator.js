...
class CollectiveValidator {
  ...
  async validateHooks() {
    ...
    // v3.0: Updated hooks list (removed obsolete hooks)
    const expectedHooks = [
      'tdd-gate.sh',
      'session-init.sh',
  ...
  async validateSettings() {
    ...
    try {
      ...
      // Check required structure (v3.0: removed SubagentStop requirement)
      ...
      if (!hasHooks || !hasSessionStart || !hasPreToolUse || !hasPostToolUse) {
        return {
          name: 'Settings JSON structure',
          passed: false,
          error: 'settings.json missing required hook configuration (SessionStart, PreToolUse, PostToolUse)'
        };
      }
