...
class CollectiveInstaller {
  constructor(options = {}) {
    ...
    this.configAdapter = new ConfigAdapter(this.platformAdapter);
    
    this.collectiveDir = path.join(
      this.projectDir, 
      this.platformAdapter.getConfigDirectory()
    );
    ...
    this.config = {
      ...
      configDir: this.platformAdapter.getConfigDirectory()
    };
  }

  async install() {
    const platformName = this.platformAdapter.isQoder() ? 'Qoder CLI' : 'Claude Code';
    console.log(chalk.cyan(`🚀 Installing AI Code Collective for ${platformName}...\n`));
  ...
  async performStandardInstallation() {
    ...
    const platformName = this.platformAdapter.isQoder() ? 'Qoder CLI' : 'Claude Code';
    const configDir = this.platformAdapter.getConfigDirectory();
    
    console.log(chalk.green('\n✅ Installation complete!'));
    console.log(chalk.yellow('\nNext steps:'));
    console.log('1. Review CLAUDE.md for behavioral directives');
    console.log(`2. Check ${configDir}/settings.json for platform configuration`);
    console.log('3. Test agent routing with a simple request');
    console.log('4. Validate: npx claude-code-collective validate');
    
    if (this.options.platform === 'both' || !this.options.platform) {
      console.log(chalk.cyan(`\n💡 Tip: This installation is configured for ${platformName}`));
      console.log(chalk.cyan('   Use --platform=qoder or --platform=claude to specify explicitly'));
    }
  ...
  async createDirectories() {
    ...
    spinner.succeed(`Directory structure created (${configDir})`);
  }
  ...
  processTemplate(content, variables) {
    let processed = this.platformAdapter.processTemplateVariables(content, {
      ...variables,
      PROJECT_ROOT: this.projectDir,
      INSTALL_DATE: this.config.installDate,
      VERSION: this.config.version,
      USER_NAME: this.config.userName,
      PROJECT_NAME: this.config.projectName,
      PLATFORM: this.platformAdapter.getPlatform(),
      CONFIG_DIR: this.platformAdapter.getConfigDirectory()
    });
  ...
  async configureSettings() {
    ...
    try {
      await this.configAdapter.createPlatformSettings(this.projectDir);
      
      if (this.options.syncPlatforms) {
        await this.configAdapter.syncSettingsAcrossPlatforms(this.projectDir);
        spinner.succeed('Settings configured and synced across platforms');
      } else {
        spinner.succeed(`Settings configured for ${this.platformAdapter.getPlatform()}`);
      }
    } catch (error) {
  ...
  async validateInstallation() {
    ...
    const checks = [
      ...
      { name: 'Settings configured', path: `${configDir}/settings.json` },
      { name: 'Hooks directory', path: `${configDir}/hooks` },
      { name: 'Agents directory', path: `${configDir}/agents` },
    ...
    const platformInfo = await this.platformAdapter.getPlatformInfo();
    const compatCheck = await this.platformAdapter.checkPlatformCompatibility(this.projectDir);
    
    if (!compatCheck.compatible) {
      allPassed = false;
      compatCheck.issues.forEach(issue => {
        results.push({ name: `Platform: ${issue}`, passed: false });
      });
    }
    
    if (allPassed) {
      spinner.succeed(`Installation validated for ${platformInfo.platform}`);
    } else {
      ...
      if (compatCheck.warnings.length > 0) {
        console.log(chalk.yellow('\n⚠️  Warnings:'));
        compatCheck.warnings.forEach(warn => console.log(`  • ${warn}`));
      }
