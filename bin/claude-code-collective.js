...
program
  ...
  .description('AI Code Collective framework - Multi-platform support for Claude Code and Qoder CLI')
...
program
  ...
  .option('--platform <platform>', 'Target platform: claude, qoder, or both', 'auto')
  .option('--sync-platforms', 'Sync configuration across both platforms')
  ...
  .action(async (path, options) => {
    try {
      ...
      const validPlatforms = ['auto', 'claude', 'qoder', 'both'];
      if (!validPlatforms.includes(options.platform)) {
        console.error(chalk.red(`❌ Invalid platform: ${options.platform}`));
        console.error(chalk.yellow(`Valid options: ${validPlatforms.join(', ')}`));
        process.exit(1);
      }

      // Express mode (--yes flag)
      if (options.yes) {
        const platformName = options.platform === 'auto' ? 'detected platform' : options.platform;
        console.log(chalk.cyan(`🚀 AI Code Collective Express Installation (${platformName})\n`));
        ...
        const installer = new CollectiveInstaller({
          ...
          syncPlatforms: options.syncPlatforms,
          targetPath: path
        });
        ...
        console.log('1. Review platform-specific settings configuration');
...
${chalk.yellow('Examples:')}
  ...
  ${chalk.gray('# Express installation for Claude Code')}
  claude-code-collective install --yes --platform=claude

  ${chalk.gray('# Express installation for Qoder CLI')}
  claude-code-collective install --yes --platform=qoder

  ${chalk.gray('# Install for both platforms')}
  claude-code-collective install --platform=both --sync-platforms
...
${chalk.yellow('Platform Options:')}
  ${chalk.gray('auto:')}   Auto-detect current platform (default)
  ${chalk.gray('claude:')} Install for Claude Code only
  ${chalk.gray('qoder:')}  Install for Qoder CLI only
  ${chalk.gray('both:')}   Install for both platforms
